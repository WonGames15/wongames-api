...
async function createSingleGame(product) {
...
export async function createGames(products, isProd = false) {
  if (isProd) {
    for (const product of products) {
      await createSingleGame(product);
    }
  } else {
    await Promise.all(products.map(createSingleGame));
  }
}
